...
Bool SPREAD_vCalculate(SPREADAPI_ttSpreadIDX tSpreadIDX)
{
	...
	psint32 ps32Spread = NULL; psint16 ps16Spread; psint8 ps8Spread;
	uint16 u16SpreadIDX; uint16 u16OldSpreadIDX; uint16 u16TempSpreadIDX;
	uint32 u32Source;
	uint16 u16Source;
	sint32 s32Source = 0; sint16 s16Source;
	sint32 s32Step;
	sint32 s32Increment;
	uint32 u32Shift = 0;
	...
	if ((0 <= tSpreadIDX) &&
	   (((sint16)SPREAD_nKernelSpreadCount + (sint16)SPREAD_nUserSpreadCount) > tSpreadIDX))
    {
		...
		switch (pstSpreadCB->enDataType)
		{
			...
			case TYPE_enUInt16:
			{
				...
				{				
					...
					s32Increment = s32Source - *(pu16Spread + SPREAD_astSpreadResult[tSpreadIDX].uSpreadData.stSpreadResult.u16SpreadIndex);
